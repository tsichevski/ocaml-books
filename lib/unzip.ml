...
module Log = (val Logs.src_log (Logs.Src.create "unzip" ~doc:"Unzipping utilities.") : Logs.LOG)
...
let extract_fb2_files ?(overwrite:bool = true) zip_path target_dir : (string list, string) result =
  ...
  if size > 4_500_000_000 then begin
    ...
    Log.info (fun m -> m  "Large archive (%.1f GB) detected — using external 7z\n"
          ...
          if Filename.check_suffix name ".fb2" then
            ...
            | Ok data ->
              ...
              if not overwrite && Sys.file_exists out_path then begin
                Log.info (fun m -> m  "Skipping existing file (overwrite=false): %s\n" out_path);
                acc
              end else begin
                ...
                Log.info (fun m -> m  "Extracted: %s\n" basename);
                out_path :: acc
              end

            | Error e ->
              Log.err (fun m -> m  "Failed to extract %s: %s\n" name e);
              acc
          else
            acc
      ) zip []
